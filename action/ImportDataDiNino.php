...
// 		if($_REQUEST['user'] == 'admin' && $_REQUEST['pwd'] == 'f7b44cfafd5c52223d5498196c8a2e7b' && $_SERVER['HTTP_STREAM_DEMO_INTEGRATION'] == 'f7b44cfafd5c52223d5498196c8a2e7b') //pwd = md5('stream')
	...
	function importContent($File)
	{
		...
		while ( ($data = fgetcsv($fh, 1000, $separator)) !== false)
		{
			$result = mysql_query("SELECT * FROM `gruppi_merceologici` WHERE gruppo = '".$data[3]."'", $conn->connection);
			if(!$row=mysql_fetch_assoc($result))
				mysql_query("INSERT INTO `gruppi_merceologici` (gruppo) VALUES ('" . $data[3]. "')", $conn->connection);
			
			$result = mysql_query("SELECT * FROM `famiglie` WHERE famiglia = '".$data[4]."'", $conn->connection);
			if(!$row=mysql_fetch_assoc($result))
				mysql_query("INSERT INTO `famiglie` (codice_famiglia, famiglia) VALUES ('".substr($data[4], 0, 3)."', '".$data[4]."')", $conn->connection);
			...
			if($row=mysql_fetch_assoc($result))
			{
				if($row['prezzo_0'] != str_replace(',', '.', $data[8]))
					$to_uodate = true;
				if($row['prezzo_1'] != str_replace(',', '.', $data[10]))
					$to_uodate = true;

				if($to_uodate)
				{
					$query = "UPDATE content SET prezzo_0 = '".str_replace(',', '.', $data[8])."', prezzo_1 = '".str_replace(',', '.', $data[10])."' WHERE id = ".$row['id'];
					mysql_query($query, $conn->connection);
					$query = null;
					$key_upd_prod++;
				}
			...
			if($row=mysql_fetch_assoc($result))
			{
				if($row['quantita'] != $data[7])
					$to_uodate = true;
				if($row['disponibilita'] != $data[6])
					$to_uodate = true;
				if($to_uodate)
				{
					$query = "UPDATE giacenze SET quantita = ".$data[7].", disponibilita = ".$data[6]." WHERE id = ".$row['id'];
					mysql_query($query, $conn->connection);
					$query = null;
					$key_upd_giac++;
				}
	...
	function importArticoli($File)
	{
		...
		while ( ($data = fgetcsv($fh, 1000, $separator)) !== false)
		{
			$result = mysql_query("SELECT * FROM `gruppi_merceologici` WHERE gruppo = '".$data[2]."'", $conn->connection);
			if(!$row=mysql_fetch_assoc($result))
				mysql_query("INSERT INTO `gruppi_merceologici` (gruppo) VALUES ('" . $data[2]. "')", $conn->connection);

			$result = mysql_query("SELECT * FROM `famiglie` WHERE famiglia = '".$data[3]."'", $conn->connection);
			if(!$row=mysql_fetch_assoc($result))
				mysql_query("INSERT INTO `famiglie` (codice_famiglia, famiglia) VALUES ('".substr($data[3], 0, 3)."', '".$data[3]."')", $conn->connection);
			...
			if($row=mysql_fetch_assoc($result))
			{
				...
				if($to_uodate)
				{
					$query = "UPDATE content SET prezzo_0 = '".str_replace(',', '.', $data[4])."', prezzo_1 = '".str_replace(',', '.', $data[5])."' WHERE id = ".$row['id'];
					mysql_query($query, $conn->connection);
					$query = null;
					$key_upd_prod++;
				}
