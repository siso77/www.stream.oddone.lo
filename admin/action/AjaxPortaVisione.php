...
class AjaxPortaVisione extends DBSmartyAction
{
	function AjaxPortaVisione()
	{
			...
			case 'cerca_libro':
				...
				if(empty($_REQUEST['pageID']))
				{
					...
					if(!empty($_REQUEST['documento_carico']))
						$where .= " magazzino.documento_carico LIKE '%".$_REQUEST['documento_carico']."%' AND ";
...
//				$excludedField[] = 'data_inserimento_riga';
			...
			case 'cerca_cliente':
				...
				if(empty($_REQUEST['pageID']))
				{
					...
					if(!empty($where))
						$where = " AND ".substr($where, 0, -4);
					$result = $Bean->dbSearch($this->conn, $where, new emails(), new emails_clienti());
