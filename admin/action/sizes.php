...
class sizes extends BeanBase
{
	var $id;
	var $id_type;
	var $size;

	function sizes($conn=null, $id=null)
	{
		parent::BeanBase();
	...
	function getView()
	{
		return 'SELECT size_type.type, sizes.id, sizes.id_type, sizes.size FROM sizes INNER JOIN size_type ON sizes.id_type = size_type.id';
	}
	
	function dbGetAllByIdSizeType($db=null, $id_size_type, $order_by = null, $order_type = null)
	{
		if (!$this->_is_connection($db))
			return false;
		...
		$query=$this->getView()." WHERE size_type.id = ".$id_size_type.' '.$params;
	...
	function dbGetAllByIdSizeType2($db=null, $id_size_type, $order_by = null, $order_type = null)
	{
		if (!$this->_is_connection($db))
			return false;
		...
		if(!empty($order_by))
			$params = " ORDER BY ". $order_by." ".$order_type;
		$query="SELECT* FROM sizes WHERE id_type = ".$id_size_type.' '.$params;
_dump($query);
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
	...
	function dbSearch($db=null, $search=null)
	{
		...
		$query = "SELECT * FROM ".$this->table_name.$search;
		$result=$db->query($query);
