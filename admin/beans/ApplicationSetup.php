...
class ApplicationSetup extends BeanBase
{
	var $id;
	var $name;
	var $description;
	...
	function ApplicationSetup($conn=null, $id=null)
	{
		parent::BeanBase();
		
		$this->table_name = "setup";
	...
	function dbGetAllByField($db=null, $field)
	{
		if (!$this->_is_connection($db))
			return false;

		$values=array();
		$query="SELECT id, description, name FROM ".$this->table_name." WHERE field_name = '".$field."' AND is_active = 1 ORDER BY name ASC";
	...
	function dbGetAllByFieldAndName($db=null, $field, $name)
	{
		if (!$this->_is_connection($db))
			return false;

		$values=array();
		$query="SELECT id, description, name FROM ".$this->table_name." WHERE field_name = '".$field."' AND name = '".$name."' AND is_active = 1 ORDER BY name ASC";
	...
	function getField_name(){return $this->field_name;}

	function setField_name($value= null)
	{
				if(strlen($value) > 255)
			$value = substr($value, 0, 255);
