...
$query="SELECT
		...
		while($row=$result->fetchRow(DB_FETCHMODE_ASSOC))
		{
			$res=$db->query("SELECT * FROM content_adhoc WHERE vbn = '".$row['bar_code']."'");
...
// 				WHERE content.is_active = 1".$search;
	...
	function dbSearchCountedDisponibili($db=null, $search=null)
	{
		...
		$query="SELECT
				...
				INNER JOIN famiglie ON content.id_famiglia = famiglie.id
				INNER JOIN gruppi_merceologici ON content.id_gm = gruppi_merceologici.id
				INNER JOIN giacenze ON content.id = giacenze.id_content
				WHERE content.is_active = 1 AND giacenze.disponibilita >= 0 AND content.prezzo_0 > 0 ".$search;
