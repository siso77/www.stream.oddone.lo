...
class emails extends BeanBase
{
	...
	function emails($conn=null, $id=null)
	{
		parent::BeanBase();
	...
	function dbGetAllByIdsEmail($db=null, array $ids)
	{
		if (!$this->_is_connection($db))
			return false;

		$values=array();
		$query="SELECT * FROM ".$this->table_name." WHERE is_active = 1 AND id IN(".implode(',', $ids).")";
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
