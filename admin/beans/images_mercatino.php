...
class images_mercatino extends BeanBase
{
	var $id;
	var $id_mercatino_content;
	var $name;
	var $local_path;
	var $www_path;

	function images_mercatino($conn=null, $id=null)
	{
		parent::BeanBase();

		$this->table_name = "images_mercatino";
	...
	function dbGetAllByIdContent($db=null, $id)
	{
		if (!$this->_is_connection($db))
			return false;

		$values=array();
		$query="SELECT * FROM ".$this->table_name." WHERE images_mercatino.id_mercatino_content = ".$id ."";
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
...
$this->setID($id);
	...
	function setId($value= null)
	{
		...
		function getId_mercatino_content(){return $this->id_mercatino_content;}

	function setId_mercatino_content($value= null)
	{
				if(strlen($value) > 10)
			$value = substr($value, 0, 10);
				
								
		$this->id_mercatino_content = (int)$value;
	}
