...
class index_fattura extends BeanBase
{
	var $id;
	var $year;

	function index_fattura($conn=null, $id=null)
	{
		parent::BeanBase();
		
		$this->table_name = "index_fattura";
	...
	function fast_edit($db, $id)
	{
		if(!$this->_is_connection($db))
			return false;
		$data = $this->dbGetAll($db);
		$query="UPDATE ".$this->table_name." SET id='".($id+1)."'";
		$db->query($query);
	...
	function custom_update($db, $id)
	{
		if(!$this->_is_connection($db))
			return false;
		$data = $this->dbGetAll($db);
		$query="UPDATE ".$this->table_name." SET id='".$id."'";
		$db->query($query);
