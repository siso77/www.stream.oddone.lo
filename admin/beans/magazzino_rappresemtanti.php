...
class magazzino_rappresemtanti extends BeanBase
{
	var $id;
	var $id_rappresentante;
	var $id_contenuto;
	var $id_tipo_presa_carico;
	var $id_distributore;
	var $quantita;
	var $documento_carico;
	...
	function magazzino_rappresemtanti($conn=null, $id=null)
	{
		parent::BeanBase();
		
		$this->table_name = "magazzino_rappresemtanti";
	...
	function getView()
	{
		return '
				SELECT 
					...
					contenuti.prezzo, 
					magazzino_rappresemtanti.id,
					magazzino_rappresemtanti.id as id_magazzino,  
					magazzino_rappresemtanti.documento_carico, 
					magazzino_rappresemtanti.percentuale_sconto, 
					magazzino_rappresemtanti.copie_omaggio, 
					magazzino_rappresemtanti.data_carico, 
					magazzino_rappresemtanti.quantita, 
					magazzino_rappresemtanti.data_inserimento_riga,
					magazzino_rappresemtanti.data_modifica_riga,  
					...
					FROM 
						(
							(
								(								
									(contenuti INNER JOIN magazzino_rappresemtanti ON magazzino_rappresemtanti.id_contenuto = contenuti.id) 
									INNER JOIN contenuti_tipo ON contenuti.id_contenuto_tipo = contenuti_tipo.id
								) 
								INNER JOIN distributore ON magazzino_rappresemtanti.id_distributore = distributore.id
							) 
	...
	function dbSearch($db=null, $search=null)
	{
		...
		$query = $this->getView()." WHERE magazzino_rappresemtanti.is_active = 1 ";
	
		$result=$db->query($query.$search);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
	...
	function getId_tipo_presa_carico(){return $this->id_tipo_presa_carico;}

	function setId_tipo_presa_carico($value= null)
	{
				if(strlen($value) > 11)
			$value = substr($value, 0, 11);
				
								
		$this->id_tipo_presa_carico = (int)$value;
	}
	...
	function getDocumento_carico(){return $this->documento_carico;}

	function setDocumento_carico($value= null)
	{
				if(strlen($value) > 255)
			$value = substr($value, 0, 255);
				
								
		$this->documento_carico = (string)$value;
	}
	...
	function getCopie_omaggio(){return $this->copie_omaggio;}

	function setCopie_omaggio($value= null)
	{
				if(strlen($value) > 11)
			$value = substr($value, 0, 11);
				
								
		$this->copie_omaggio = (int)$value;
	}

	function getData_carico(){return $this->data_carico;}

	function setData_carico($value= null)
	{
				
				if(strrpos($value, "-") != 7)
		{
			echo "Errore nel settaggio della properies data_carico!";
			exit;
		}
								
		$this->data_carico = (string)$value;
	}
