...
class stolen_content extends BeanBase
{
	var $id;
	var $id_user;
	var $name;
	...
	function stolen_content($conn=null, $id=null)
	{
		parent::BeanBase();

		$this->table_name = "stolen_content";
	...
	function dbGetAllCustom($db=null, $BeanImagesMercatino = null)
	{
		...
		$values=array();
		$query="SELECT * FROM ".$this->table_name." ORDER BY is_publish ASC";
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
	...
	function dbGetOneCustom($db=null, $id, $BeanImagesMercatino = null)
	{
		if (!$this->_is_connection($db))
			return false;

		$values=array();
		$query="SELECT * FROM ".$this->table_name." WHERE id = ".$id." ORDER BY is_publish ASC";
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
