...
//				$menu[$i]['text'] = $text[$val];
	...
	function getImageFromIdContent($id, $dimension = 'Small_')
	{
		...
		if($images != array() && $images[0]['name'] != 'default.jpg')
			return '<img src="'.$images[0]['www_path'].'/'.$dimension.$images[0]['name'].$images[0]['ext'].'" alt="'.$images[0]['name'].'">';
	}

	function getImagePathFromIdContent($id, $dimension = '')
	{
		include_once(APP_ROOT.'/beans/images.php');
		$BeanImages = new images();
		$images = $BeanImages->dbGetAllByIdContent(MyDB::connect(), $id);
...
//			}
	...
	function getFornitoreFromId($id)
	{
		include_once(APP_ROOT.'/beans/fornitore.php');
		$Bean = new fornitore(MyDB::connect(), $id);
		$data = $Bean->vars();
