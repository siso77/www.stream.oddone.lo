...
class brands extends BeanBase
{
	var $id;
	var $name;
	var $link;

	function brands($conn=null, $id=null)
	{
		parent::BeanBase();
	...
	function dbGetAllCountContent($db=null, $order_by = null, $order_type = null, $BeanContent)
	{
		if (!$this->_is_connection($db))
			return false;
		...
		while($row=$result->fetchRow(DB_FETCHMODE_ASSOC))
		{
			$numContent = $BeanContent->dbGetCountContent($db, ' AND content.id_brand = '.$row['id']);
