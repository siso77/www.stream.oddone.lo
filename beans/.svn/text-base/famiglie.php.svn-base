...
class famiglie extends BeanBase
{
	var $id;
	var $codice_famiglia;
	var $famiglia;

	function famiglie($conn=null, $id=null)
	{
		parent::BeanBase();

		$this->table_name = "famiglie";
	...
	function dbGetByFamiglia($db=null, $famiglia = null)
	{
		if (!$this->_is_connection($db))
			return false;

		$values=false;
		$query="SELECT * FROM ".$this->table_name." WHERE famiglia = '".$famiglia."'";
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
	...
	function dbGetByCodFamiglia($db=null, $cod = null)
	{
		if (!$this->_is_connection($db))
			return false;

		$values=false;
		$query="SELECT * FROM ".$this->table_name." WHERE codice_famiglia = '".$cod."'";
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
...
$this->setID($id);
	...
	function getFamiglia(){return $this->famiglia;}

	function setFamiglia($value= null)
	{
				if(strlen($value) > 255)
			$value = substr($value, 0, 255);
				
								
		$this->famiglia = (string)$value;
	}
	
	function getCodice_famiglia(){return $this->codice_famiglia;}

	function setCodice_famiglia($value= null)
	{
		if(strlen($value) > 255)
			$value = substr($value, 0, 255);
				
								
		$this->codice_famiglia = (string)$value;
	}
