...
class images_brands extends BeanBase
{
	var $id;
	var $id_brand;
	var $name;
	...
	function images_brands($conn=null, $id=null)
	{
		parent::BeanBase();
		
		$this->table_name = "images_brands";
	...
	function dbGetAllByIdBrand($db=null, $idBrand)
	{
		if (!$this->_is_connection($db))
			return false;

		$values=array();
		$query="SELECT * FROM ".$this->table_name." WHERE id_brand = ".$idBrand;
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
