...
class images_stolen extends BeanBase
{
	var $id;
	var $id_stolen_content;
	var $name;
	var $local_path;
	var $www_path;

	function images_stolen($conn=null, $id=null)
	{
		parent::BeanBase();

		$this->table_name = "images_stolen";
	...
	function dbGetAllByIdContent($db=null, $id)
	{
		...
		$values=array();
		$query="SELECT * FROM ".$this->table_name." WHERE images_stolen.id_stolen_content = ".$id ."";
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
...
$this->setID($id);
	...
	function setId($value= null)
	{
		...
		function getId_stolen_content(){return $this->id_stolen_content;}

	function setId_stolen_content($value= null)
	{
				if(strlen($value) > 10)
			$value = substr($value, 0, 10);
				
								
		$this->id_stolen_content = (int)$value;
	}
