...
class contenuti extends BeanBase
{
	var $id;
	var $id_casa_editrice;
	var $id_autore;
	var $id_contenuto_tipo;
	var $isbn;
	var $titolo;
	var $descrizione;
	var $prezzo;
	...
	function contenuti($conn=null, $id=null)
	{
		parent::BeanBase();
		
		$this->table_name = "contenuti";
	...
	function dbGetOneByIsbn($db=null, $isbn=null)
	{
		if (!$this->_is_connection($db))
			return false;

		$query="SELECT * FROM ".$this->table_name." WHERE isbn LIKE '%". $isbn . "%' AND is_active = 1";		
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
	...
	function dbGetAllCustomField($db=null, $field)
	{
		...
		$values=array();
		$query="SELECT ".$field." AS data FROM ".$this->table_name." WHERE is_active = 1";
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
	...
	function getId_casa_editrice(){return $this->id_casa_editrice;}

	function setId_casa_editrice($value= null)
	{
				if(strlen($value) > 11)
			$value = substr($value, 0, 11);
				
								
		$this->id_casa_editrice = (int)$value;
	}

	function getId_autore(){return $this->id_autore;}

	function setId_autore($value= null)
	{
				if(strlen($value) > 11)
			$value = substr($value, 0, 11);
				
								
		$this->id_autore = (int)$value;
	}

	function getId_contenuto_tipo(){return $this->id_contenuto_tipo;}

	function setId_contenuto_tipo($value= null)
	{
				if(strlen($value) > 11)
			$value = substr($value, 0, 11);
				
								
		$this->id_contenuto_tipo = (int)$value;
	}

	function getIsbn(){return $this->isbn;}

	function setIsbn($value= null)
	{
				if(strlen($value) > 255)
	...
	function setPrezzo($value= null)
	{
				if(strlen($value) > 255)
			$value = substr($value, 0, 255);
				
								
		$this->prezzo = (string)$value;
	}

	function getNumero(){return $this->numero;}

	function setNumero($value= null)
	{
				if(strlen($value) > 255)
			$value = substr($value, 0, 255);
	...
	function getEdizione(){return $this->edizione;}

	function setEdizione($value= null)
	{
		if(strlen($value) > 255)
			$value = substr($value, 0, 255);
				
								
		$this->edizione = (string)$value;
	}
