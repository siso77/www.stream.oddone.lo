...
class contenuti_tipo extends BeanBase
{
	var $id;
	var $tipo;
	...
	function contenuti_tipo($conn=null, $id=null)
	{
		parent::BeanBase();
		
		$this->table_name = "contenuti_tipo";
	...
	function dbGetAllForSetup($db=null)
	{
		if (!$this->_is_connection($db))
			return false;

		$values=array();
		$query="SELECT id, tipo as name FROM ".$this->table_name." WHERE is_active = 1";
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
