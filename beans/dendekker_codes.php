...
class dendekker_codes extends BeanBase
{
	var $id;
	var $code;
	var $customer_code;

	function dendekker_codes($conn=null, $id=null)
	{
		parent::BeanBase();

		$this->table_name = "dendekker_codes";
	...
	function dbGetOneByCustomerCode($db=null, $customer_code=null)
	{
		if (!$this->_is_connection($db))
			return false;
	
		$query="SELECT * FROM ".$this->table_name." WHERE customer_code='". $customer_code ."'";
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
...
$this->setID($id);
	...
	function setCustomer_code($value= null)
	{
		...
		$this->customer_code = (string)$value;
	}
