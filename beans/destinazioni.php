...
class destinazioni extends BeanBase
{
	var $id;
	var $codice_etiflor;
	var $customer_code;
	...
	function destinazioni($conn=null, $id=null)
	{
		parent::BeanBase();
		
		$this->table_name = "destinazioni";
	...
	function dbGetAllByCustomerCode($db=null, $customer_code)
	{
		if (!$this->_is_connection($db))
			return false;
	
		$values=array();
		$query="SELECT * FROM ".$this->table_name." WHERE is_active = 1 AND customer_code LIKE '".$customer_code."%'";
...
// 			$value = substr($value, 0, 11);
	...
	function getCodice_etiflor(){return $this->codice_etiflor;}
	
	function setCodice_etiflor($value= null)
	{
		// 		if(strlen($value) > 11)
			// 			$value = substr($value, 0, 11);
	
			$this->codice_etiflor = $value;
	}
}
?>