...
class images_color extends BeanBase
{
	var $id;
	var $id_color;
	var $name;
	...
	function images_color($conn=null, $id=null)
	{
		parent::BeanBase();
		
		$this->table_name = "images_color";
	...
	function dbGetAllByIdColor($db=null, $id)
	{
		if (!$this->_is_connection($db))
			return false;

		$values=array();
		//$query="SELECT * FROM ".$this->table_name." WHERE id_color = ".$idBrand;
		$query = "SELECT color.color, images_color.name, images_color.local_path, images_color.www_path, color.id, images_color.id as id_color
					FROM images_color 
					INNER JOIN color ON images_color.id_color = color.id = ".$id;
