...
class reso_post_vendita extends BeanBase
{
	var $id;
	var $id_cliente;
	...
	function reso_post_vendita($conn=null, $id=null)
	{
		parent::BeanBase();
		
		$this->table_name = "reso_post_vendita";
	...
	function getAllByIdMagazzino($db=null, $idMagazzino)
	{
		if (!$this->_is_connection($db))
			return false;

		$values=array();
		$query="SELECT * FROM ".$this->table_name." WHERE is_active = 1 AND id_magazino = ".$idMagazzino;
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
	...
	function dbGetVendite($db=null, $search=null)
	{
		if (!$this->_is_connection($db))
			return false;
		...
		$query = $this->getViewVendite()." AND ".$this->table_name.".is_active = 1 ".$search;
		$result=$db->query($query);
	...
	function getView()
	{
		return "SELECT 
					...
					FROM 
						...
						distributore 
					WHERE 
						vendite.id_rappresentante = rappresentante.id AND 
						vendite.id_rif_new_age = riferimento_new_age.id AND 
						vendite.id_magazino = magazzino.id AND 
						vendite.id_cliente = clienti.id AND 						
							...
							(
								(
									(
										(
											(
												(rappresentante INNER JOIN vendite ON rappresentante.id = vendite.id_rappresentante) 
												INNER JOIN magazzino ON vendite.id_magazino = magazzino.id
											) 
											INNER JOIN contenuti ON magazzino.id_contenuto = contenuti.id
										) 
										INNER JOIN contenuti_tipo ON contenuti.id_contenuto_tipo = contenuti_tipo.id
									) 
									INNER JOIN distributore ON magazzino.id_distributore = distributore.id
								) 
								INNER JOIN clienti ON vendite.id_cliente = clienti.id
							) 
	...
	function getNota_credito(){return $this->nota_credito;}

	function setNota_credito($value= null)
	{
				if(strlen($value) > 255)
			$value = substr($value, 0, 255);
				
								
		$this->nota_credito = (string)$value;
	}
