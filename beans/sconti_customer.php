...
class sconti_customer extends BeanBase
{
	var $id_sconto;
	var $customer_code;

	function sconti_customer($conn=null, $id=null)
	{
		parent::BeanBase();
		
		$this->table_name = "sconti_customer";
	...
	function dbGetAllByCustomerCode($db=null, $CustomerCode)
	{
		if (!$this->_is_connection($db))
			return false;

		$values=array();
		$query="SELECT * FROM ".$this->table_name.' WHERE customer_code='.$CustomerCode;
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
		...
		while($row=$result->fetchRow(DB_FETCHMODE_ASSOC))
		{
			$BeanSconti = new sconti($db, $row['id_sconto']);
			$row['nome'] = $BeanSconti->nome;
			$row['percentuale'] = $BeanSconti->percentuale;
			$row['colore'] = $BeanSconti->colore;
			$values[]=$row;
		}
	...
	{
		$query = "DELETE FROM ".$this->table_name." WHERE customer_code = ".$id;
	...
	/*			GET e SET		*/	
	function getId_sconto(){return $this->id_sconto;}

	function setId_sconto($value= null)
	{
		if(strlen($value) > 11)
			$value = substr($value, 0, 11);
		$this->id_sconto = (int)$value;
	}
