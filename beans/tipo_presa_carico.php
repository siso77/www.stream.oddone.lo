...
class tipo_presa_carico extends BeanBase
{
	var $id;
	var $nome;
	...
	function tipo_presa_carico($conn=null, $id=null)
	{
		parent::BeanBase();
		
		$this->table_name = "tipo_presa_carico";
	...
	function dbGetAllForSetup($db=null)
	{
		...
		$values=array();
		$query="SELECT id,nome as name FROM ".$this->table_name." WHERE is_active = 1";
		$result=$db->query($query);
		if(get_class($result) == "DB_Error")
			return $this->_showErrorNoQuery("File: ".__FILE__."<BR> Class: ".get_class($this)."<BR>Line: ".__LINE__."<BR>Query: <BR>".$query);
