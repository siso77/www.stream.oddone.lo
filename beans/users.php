...
class users extends BeanBase
{
	...
	function dbGetOneByIdAnag($db=null, $id=null)
	{
		if (!$this->_is_connection($db))
			return false;

		$query="SELECT * FROM ".$this->table_name." WHERE id_anag=". $id ."";
...
//		$query = "SELECT * FROM ".$this->table_name." WHERE `".$this->table_name."`.is_active = 1 ".$where;
	...
	function getPwd_resetted(){return $this->pwd_resetted;}

	function setPwd_resetted($value= null)
	{
		if(strlen($value) > 11)
			$value = substr($value, 0, 11);
		$this->pwd_resetted = (int)$value;
	}
