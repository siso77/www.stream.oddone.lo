...
if ($env=getenv('PHP_PEAR_INSTALL_DIR')) {
    ...
    putenv('PHP_PEAR_INSTALL_DIR=C:\Apache2\htdocs\install_pear/PEAR');
}

if ($env=getenv('PHP_PEAR_BIN_DIR')) {
    define("PHP_PEAR_BIN_DIR",$env);
} else {
    putenv('PHP_PEAR_BIN_DIR=C:\Apache2\htdocs\install_pear');
}
...
ini_set('include_path', 'C:\Apache2\htdocs\install_pear/PEAR');
$useDHTML         = true;
